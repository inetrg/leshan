...
public class LeshanClientDemo {
    ...
    public static LeshanClient createClient(LeshanClientDemoCLI cli, LwM2mModelRepository repository) throws Exception {
        ...
        if (cli.oscore != null) {
            ...
            Oscore oscoreObject = new Oscore(oscoreObjectInstanceId, cli.oscore.masterSecret, cli.oscore.senderId,
                    cli.oscore.recipientId, cli.oscore.aeadAlgorithm, cli.oscore.hkdfAlgorithm, cli.oscore.masterSalt);
        ...
        if (cli.main.bootstrap) {
            if (cli.identity.isPSK()) {
                ...
                if (oscoreObjectInstanceId != null) {
                    // TODO OSCORE change oscoreOnlyBootstrap API it should not take [Integer shortServerId] as argument
                    initializer.setInstancesForObject(SECURITY,
                            oscoreOnlyBootstrap(cli.main.url, null, oscoreObjectInstanceId));
                } else {
